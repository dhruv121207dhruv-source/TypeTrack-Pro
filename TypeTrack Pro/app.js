  ...
  if(inputVal.length>0&&state.currentIndex<state.chars.length){
    ...
    if(typedChar===expectedChar){
      ...
      if(charEl){
        ...
        if(state.glitchEnabled) triggerGlitchBurst();
      }
      sound.play('error');
    }
...
$$('#glitch-options .pill').forEach(btn=>btn.addEventListener('click',()=>{
  $$('#glitch-options .pill').forEach(b=>b.classList.remove('active'));
  btn.classList.add('active'); 
  state.glitchEnabled = btn.dataset.glitch === 'on';
  if(state.glitchEnabled) dom.practiceView.classList.add('glitch-mode-active');
  else {
    dom.practiceView.classList.remove('glitch-mode-active');
...
function handleParallax(e) {
  if (!state.glitchEnabled || !$('#view-practice').classList.contains('active')) return;
  ...
  const moveX = (mouseX - centerX) * friction;
  const moveY = (mouseY - centerY) * friction;
...
function handleGyroscope(e) {
  ...
  const friction = 0.5;
  const moveX = (e.gamma || 0) * friction; 
  const moveY = (e.beta || 0) * friction;  
...
function triggerGlitchBurst() {
  const intensity = Math.min(state.errors * 0.4, 6);
  document.documentElement.style.setProperty('--glitch-intensity', intensity);
  
  const wrapper = $('.text-display-wrapper');
  if (wrapper) {
    wrapper.classList.add('glitch-error-burst');
    setTimeout(() => {
      wrapper.classList.remove('glitch-error-burst');
...
window.addEventListener('touchstart', (e) => {
  if (state.glitchEnabled) handleParallax(e);
}, { passive: true });
